...
DARC_DIRECTORY_VER5* DXArchive_VER5::GetCurrentDirectoryInfo(void) { return CurrentDirectory; }
	...
	for (; Src[i] != '\0' && Src[i] != '\\'; )
	{
		if (CheckMultiByteChar(&Src[i]) == TRUE)
		{
			...
			*((u16*)&SearchData->FileName[i]) = *((u16*)&Src[i]);
			ParityData += static_cast<u8>(Src[i]) + static_cast<u8>(Src[i + 1]);
			i += 2;
		}
		else
		{
			// 小文字の場合は大文字に変換
			if (Src[i] >= 'a' && Src[i] <= 'z')	SearchData->FileName[i] = static_cast<u8>(Src[i]) - 'a' + 'A';
			else SearchData->FileName[i] = Src[i];
			ParityData += static_cast<u8>(SearchData->FileName[i]);
			i++;
		}
	}
...
int DXArchive_VER5::AddFileNameData(const TCHAR* FileName, u8* FileNameTable)
{
	...
	PackNum = static_cast<int>((Length + 3) / 4);

	// パック数を保存
	*((u16*)&FileNameTable[0]) = static_cast<u16>(PackNum);
	...
	for (i = 0; FileName[i] != '\0'; )
	{
		...
		if (CheckMultiByteChar(&FileName[i]) == true)
		{
			...
			*((u16*)&FileNameTable[4 + i]) = *((u16*)&FileName[i]);
			Parity += static_cast<u8>(FileName[i]) + static_cast<u8>(FileName[i + 1]);
			i += 2;
		}
		else
		{
			...
			{
				// 小文字の場合は大文字に変換
				FileNameTable[4 + i] = static_cast<u8>(FileName[i]) - 'a' + 'A';
			}
			else
			{
				// そうではない場合は普通にコピー
				FileNameTable[4 + i] = static_cast<u8>(FileName[i]);
			}
	...
	// パリティ情報を保存
	*((u16*)&FileNameTable[2]) = static_cast<u16>(Parity);
...
{
	int DwordNum{ Size / 4 };
	int ByteNum{ Size - DwordNum * 4 };

#ifndef INLINE_ASM
	{
		//		int i ;
		DWORD* dd{ static_cast<DWORD*>(Data) };
...
void DXArchive_VER5::NotConvFileWrite(void* Data, const int Size, FILE* fp)
{
	// データを反転する
	NotConv(Data, Size);
...
void DXArchive_VER5::NotConvFileRead(void* Data, const int Size, FILE* fp)
{
	// 読み込む
...
{
	if (Source == NULL) { memset(Key, 0xaaaaaaaa, DXA_KEYSTR_LENGTH_VER5); }
	else
	{
		size_t Len{ strlen(Source) };
		if (Len > DXA_KEYSTR_LENGTH_VER5) { memcpy(Key, Source, DXA_KEYSTR_LENGTH_VER5); }
		else
		{
			// 鍵文字列が DXA_KEYSTR_LENGTH_VER5 より短かったらループする
			size_t i{};

			for (; i + Len <= DXA_KEYSTR_LENGTH_VER5; i += Len) memcpy(Key + i, Source, Len);

			if (i < DXA_KEYSTR_LENGTH_VER5) memcpy(Key + i, Source, DXA_KEYSTR_LENGTH_VER5 - i);
		}
	}
...
void DXArchive_VER5::KeyConv(void* Data, const int Size, int Position, unsigned char* Key)
{
...
void DXArchive_VER5::KeyConvFileWrite(void* Data, const int Size, FILE* fp, unsigned char* Key, const int Position)
{
	// ファイルの位置を取得しておく
	int pos{ Position == -1 ? ftell(fp) : Position };
...
void DXArchive_VER5::KeyConvFileRead(void* Data, const int Size, FILE* fp, unsigned char* Key, const int Position)
{
	// ファイルの位置を取得しておく
	int pos{ Position == -1 ? ftell(fp) : Position };
	//if (Position == -1) pos = ftell(fp);
...
int DXArchive_VER5::DirectoryEncode(TCHAR* DirectoryName, u8* NameP, u8* DirP, u8* FileP, DARC_DIRECTORY_VER5* ParentDir, SIZESAVE* Size, int DataNumber, FILE* DestP, void* TempBuffer, bool Press, unsigned char* Key)
{
	...
	// ディレクトリの情報を得る
	HANDLE FindHandle{ FindFirstFile(DirectoryName, &FindData) };
	...
	DARC_FILEHEAD_VER5 File
	{
		...
		File.Time.Create = ((static_cast<LONGLONG>(FindData.ftCreationTime.dwHighDateTime)) << 32) + FindData.ftCreationTime.dwLowDateTime,
		File.Time.LastAccess = ((static_cast<LONGLONG>(FindData.ftLastAccessTime.dwHighDateTime)) << 32) + FindData.ftLastAccessTime.dwLowDateTime,
		File.Time.LastWrite = ((static_cast<LONGLONG>(FindData.ftLastWriteTime.dwHighDateTime)) << 32) + FindData.ftLastWriteTime.dwLowDateTime,
	...
	// ディレクトリ情報のセット
	{
		...
		else { Dir.ParentDirectoryAddress = 0; }
	...
	// ファイル情報を出力する
	{
		...
		do
		{
			...
			else
			{
				...
				// ファイルデータを書き出す
				if (static_cast<u64>(FindData.nFileSizeLow) + (static_cast<u64>(FindData.nFileSizeHigh) << 32) != 0)
				{
					u32 WriteSize;
					u32 MoveSize;

					// ファイルを開く
					FILE* SrcP{ _tfopen(FindData.cFileName, TEXT("rb")) };

					// サイズを得る
					fseek(SrcP, 0, SEEK_END);
					u32 FileSize{ static_cast<u32>(ftell(SrcP)) };
					fseek(SrcP, 0, SEEK_SET);

					// ファイルサイズが 10MB 以下の場合で、圧縮の指定がある場合は圧縮を試みる
					if (Press == true && static_cast<u64>(FindData.nFileSizeLow) + (static_cast<u64>(FindData.nFileSizeHigh) << 32) < 10 * 1024 * 1024)
					{
						void* SrcBuf;
						void* DestBuf;
						u32 DestSize;
						...
						if (static_cast<f64>(DestSize) / static_cast<f64>(FileSize) > 0.90)
						{
...
int DXArchive_VER5::DirectoryDecode(u8* NameP, u8* DirP, u8* FileP, DARC_HEAD_VER5* Head, DARC_DIRECTORY_VER5* Dir, FILE* ArcP, unsigned char* Key)
{
	...
	{
		// 格納されているファイルの数だけ繰り返す
		u32 FileHeadSize{ Head->Version >= 0x0002 ? sizeof(DARC_FILEHEAD_VER5) : sizeof(DARC_FILEHEAD_VER1) };
		DARC_FILEHEAD_VER5* File{ (DARC_FILEHEAD_VER5*)(FileP + Dir->FileHeadAddress) };
		for (u32 i{}; i < Dir->FileHeadNum; i++, File = (DARC_FILEHEAD_VER5*)((u8*)File + FileHeadSize))
		{
			// ディレクトリかどうかで処理を分岐
			...
			else
			{
				...
				// バッファを確保する
				void* Buffer{ malloc(DXA_BUFFERSIZE_VER5) };
				if (Buffer == NULL) return -1;

				// ファイルを開く
				TCHAR* pName = GetOriginalFileName(NameP + File->NameAddress);
				FILE* DestP{ _tfopen(pName, TEXT("wb")) };
				delete[] pName;
				...
				{
					// 初期位置をセットする
					if (ftell(ArcP) != static_cast<s32>(Head->DataStartAddress + File->DataAddress))
					...
					if (Head->Version >= 0x0002 && File->PressDataSize != 0xffffffff)
					{
						...
						// 圧縮データが収まるメモリ領域の確保
						void* temp{ malloc(File->PressDataSize + File->DataSize) };
					...
					else
					{
						...
						// 転送処理開始
						{
							...
							{
								u32 MoveSize{ File->DataSize - WriteSize > DXA_BUFFERSIZE_VER5 ? DXA_BUFFERSIZE_VER5 : File->DataSize - WriteSize };
				...
				// ファイルのタイムスタンプを設定する
				{
					...
					if (HFile == INVALID_HANDLE_VALUE) HFile = HFile;
...
int DXArchive_VER5::Decode(const void* const Src, const void* const Dest)
{
	u8* destp{ (u8*)Dest };
	u8* srcp{ (u8*)Src };

	// 解凍後のデータサイズを得る
	const u32 destsize{ *((u32*)&srcp[0]) };

	// 圧縮データのサイズを得る
	u32 srcsize{ *((u32*)&srcp[4]) - 9 };
...
{
	// ファイルの数を取得する
	const int FileNum{ GetDirectoryFilePath(DirectoryPath, NULL) };
	if (FileNum < 0) return -1;

	// ファイルの数の分だけファイル名とファイルポインタの格納用のメモリを確保する
	TCHAR* const NameBuffer = (TCHAR*)malloc(FileNum * (256 + 4));

	// ファイルのパスを取得する
	GetDirectoryFilePath(DirectoryPath, NameBuffer);

	// ファイルパスのリストを作成する
	TCHAR** const FilePathList{ (TCHAR**)(NameBuffer + FileNum * 256) };
	for (int i{}; i < FileNum; i++)
		FilePathList[i] = NameBuffer + i * 256;

	// エンコード
	const int Result{ EncodeArchive(OutputFileName, FilePathList, FileNum, Press, KeyString) };
